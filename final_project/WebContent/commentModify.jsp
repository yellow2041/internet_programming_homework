...
<%
	...
	int idx = Integer.parseInt(request.getParameter("contentsIdx"));
	...
	<div style="position: relative; float: left;width:250px">
		...
		<article style="padding-left: 20px; position: absolute; top: 0px; left: 250px; width: 750px; float: left;">
			...
			<div style="padding-top: 10px;">
				...
				%>
				<div style="position: relative; padding: 0px 10px 10px; width: 500px;">
			...
			<div>
				...
				function confirmDelete2(contentsIdx,commentIdx) {
					...
					<%
					if(rs.getInt("idx")==commentIdx){ 
					%>
					<form action="commentModifyDo.jsp?commentIdx=<%=commentIdx%>&contentsIdx=<%=contentsIdx %>" method="post">
					<div class="form-row" style="margin-bottom: 20px">
						<textarea class="form-control" name="comment" id="exampleFormControlTextarea1" style="width: 88%; height: 60px" rows="2"><%=rs.getString("comment")%></textarea>
						<button class="btn btn-info btn-lg" type="submit" style="height: 60px; margin-left: 3px; float: right" type="submit">수 정</button>
					</div>
				</form>
				<%
					}else{
